...
BEGIN
  ...
  IF CONTEMINCIDENCIA( 'CORRECAO', 'JUROS', CONFIGURACAO_ACRESCIMOS.ID) = 1 THEN
    PARAM_VALOR                                                        := PARAM_VALOR + GETVALORCORRECAO(PARAM_ID_PARCELA, EXTRACT(YEAR FROM PARAM_DATA_VENCIMENTO));
  END IF;
  ...
  END IF;
  RETURN ROUND(VALOR_JUROS + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'JUROS'),2);
EXCEPTION
...
BEGIN
  ...
  RETURN ROUND(VALOR_MULTA         + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'MULTA'),2);
EXCEPTION
WHEN NO_DATA_FOUND THEN
  RETURN 0;
WHEN TIPO_APLIC_ACRESCIMO_EXCEPTION THEN
...
BEGIN
  ...
  INTO VALORCORRIGIDO
  FROM DUAL;
  RETURN ROUND((VALORCORRIGIDO - PARAM_SALDO) + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'CORRECAO'),2);
...
FUNCTION QUANTIDADEIMPRESSOESDAM(PARAM_PARCELA_ID IN NUMBER)
RETURN NUMBER
AS
 RETORNO           NUMBER;
BEGIN
 select count(hist.id) INTO RETORNO from
itemdam it inner join dam on dam.id = it.dam_id
