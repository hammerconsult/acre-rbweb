...
AS
    ...
    AS
        ...
        BEGIN
            ...
            IF COTA_UNICA = 1 AND PARAM_DATA_VENCIMENTO >= PARAM_DATA_OPERACAO
            THEN
                RETURN 0;
            END IF;
            IF PARAM_DATA_VENCIMENTO >= TO_DATE(PARAM_DATA_OPERACAO, 'dd/MM/yyyy')
            THEN
                VALOR_JUROS := GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'JUROS');
                IF CALCULA_DESCONTO = 1
                THEN
                    PAGTO_PARCELAMENTO := COALESCE(PGTOPARCCANCPARCELAMENTO(PARAM_ID_PARCELA), 0);
                    IF PAGTO_PARCELAMENTO >= VALOR_JUROS
                    THEN
                        VALOR_JUROS := 0;
                    ELSE
                ...
                RETURN VALOR_JUROS;
            END IF;
            IF PARAM_SITUACAO = 'PAGO' OR PARAM_SITUACAO = 'BAIXADO'
            THEN
            ...
            THEN
                ...
                RETURN ROUND(VALOR_JUROS, 2);
            END IF;
            IF PARAM_SITUACAO = 'PAGO_PARCELAMENTO'
            THEN
                SELECT COALESCE(SUM(PAGAMENTO.JUROS),0)
                INTO VALOR_JUROS
                ...
                RETURN ROUND(VALOR_JUROS, 2);
            END IF;
            IF PARAM_SITUACAO = 'PAGO_SUBVENCAO'
            THEN
                SELECT sub.JUROS
                INTO VALOR_JUROS
                ...
                RETURN ROUND(VALOR_JUROS, 2);
            END IF;
            IF PARAM_SITUACAO = 'SUBSTITUIDA_POR_COMPENSACAO'
            THEN
            ...
            THEN
                ...
                RETURN ROUND(VALOR_JUROS, 2);
            END IF;
            IF PARAM_SITUACAO <> 'EM_ABERTO'
            THEN
                RETURN GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'JUROS');
            END IF;
            SELECT
                COALESCE(MAX(PRO.ISENTAJUROS), 0),
                MAX(PRO.DATAFIM)
            INTO ISENTA_JUROS, DATA_ISENCAO_JUROS
            FROM ISENCAOACRESCIMOPARCELA ISP
                INNER JOIN PROCESSOISENCAOACRESCIMOS PRO ON PRO.ID = ISP.PROCESSOISENCAOACRESCIMOS_ID
            WHERE ISP.PARCELA_ID = PARAM_ID_PARCELA AND PRO.SITUACAO = 'ATIVO'
                  AND TRUNC(PARAM_DATA_OPERACAO) >= PRO.DATAINICIO;
            IF ISENTA_JUROS = 1 AND TRUNC(PARAM_DATA_OPERACAO) <= DATA_ISENCAO_JUROS
            THEN
                RETURN 0;
            END IF;
            IF ISENTA_JUROS = 1 AND TRUNC(PARAM_DATA_OPERACAO) > DATA_ISENCAO_JUROS
            ...
            IF CONFIGURACAO_ACRESCIMOS.ID IS NULL
            THEN
                RETURN GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'JUROS');
            END IF;
            IF CONTEMINCIDENCIA('IMPOSTO', 'JUROS', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_VALOR := PARAM_VALOR + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'IMPOSTO');
            END IF;
            IF CONTEMINCIDENCIA('TAXA', 'JUROS', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_VALOR := PARAM_VALOR + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'TAXA');
            END IF;
            IF CONTEMINCIDENCIA('CORRECAO', 'JUROS', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_VALOR := PARAM_VALOR +
                               GETVALORCORRECAO(PARAM_ID_PARCELA, EXTRACT(YEAR FROM PARAM_DATA_VENCIMENTO), 0,
                                                PARAM_DATA_OPERACAO);
            END IF;
            IF CONTEMINCIDENCIA('MULTA_ORIGINAL', 'JUROS', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_VALOR := PARAM_VALOR + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'MULTA');
            END IF;
            IF CONTEMINCIDENCIA('COREECAO_ORIGINAL', 'JUROS', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_VALOR := PARAM_VALOR + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'CORRECAO');
            END IF;
            IF CONTEMINCIDENCIA('HONORARIOS_ORIGINAL', 'JUROS', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_VALOR := PARAM_VALOR + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'HONORARIOS');
            END IF;
            IF CONTEMINCIDENCIA('JUROS_ORIGINAL', 'JUROS', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
            ...
            END IF;
            IF CONFIGURACAO_ACRESCIMOS.JUROSFRACIONADO = 1
            THEN
                MULTIPLICADOR := DIFERENCA_MESES + (DIFERENCA_DIAS / QTDADE_DIAS_MES_ANTERIOR);
            ELSE
                IF DIFERENCA_DIAS > 0
                THEN
                    IF ROUND(DIFERENCA_MESES, 0) < DIFERENCA_MESES
                    THEN
                        DIFERENCA_MESES := DIFERENCA_MESES + 1;
                    END IF;
            ...
            IF PARAM_TIPO_CALC <> 'NFSE'
            THEN
                VALOR_JUROS := ROUND(VALOR_JUROS + JUROS_ORIGINAL, 2);
            ELSE
                IF VALOR_JUROS < JUROS_ORIGINAL
                THEN
        ...
        BEGIN
            ...
            THEN
                ...
                THEN
                    ...
                    IF PAGTO_PARCELAMENTO >= JUROS
                    THEN
                        PAGTO_PARCELAMENTO := PAGTO_PARCELAMENTO - JUROS;
                        IF PAGTO_PARCELAMENTO >= VALOR_MULTA
                        THEN
                            VALOR_MULTA := 0;
                        ELSE
            ...
            IF PARAM_SITUACAO = 'PAGO_PARCELAMENTO'
            THEN
                SELECT  COALESCE(SUM(PAGAMENTO.MULTA),0)
                INTO VALOR_MULTA
                FROM PAGAMENTOPORMOVTRIBUTARIA PAGAMENTO
                WHERE parcela_id = PARAM_ID_PARCELA;
            ...
            WHERE ISP.PARCELA_ID = PARAM_ID_PARCELA AND PRO.SITUACAO = 'ATIVO'
                  AND
                  TRUNC(PARAM_DATA_OPERACAO) BETWEEN PRO.DATAINICIO AND COALESCE(DATAFIM, TRUNC(PARAM_DATA_OPERACAO));
            IF ISENTA_MULTA = 1
            THEN
                RETURN 0;
            END IF;
            ...
            THEN
                RETURN 0;
            END IF;
            IF CONTEMINCIDENCIA('IMPOSTO', 'MULTA', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_VALOR := PARAM_VALOR + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'IMPOSTO');
            END IF;
            IF CONTEMINCIDENCIA('TAXA', 'MULTA', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_VALOR := PARAM_VALOR + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'TAXA');
            END IF;
            IF CONTEMINCIDENCIA('CORRECAO', 'MULTA', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_VALOR := PARAM_VALOR +
                               GETVALORCORRECAO(PARAM_ID_PARCELA, EXTRACT(YEAR FROM PARAM_DATA_VENCIMENTO), 0,
                                                PARAM_DATA_OPERACAO);
            END IF;
            IF CONTEMINCIDENCIA('MULTA_ORIGINAL', 'MULTA', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_VALOR := PARAM_VALOR + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'MULTA');
            END IF;
            IF CONTEMINCIDENCIA('COREECAO_ORIGINAL', 'MULTA', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_VALOR := PARAM_VALOR + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'CORRECAO');
            END IF;
            IF CONTEMINCIDENCIA('HONORARIOS_ORIGINAL', 'MULTA', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_VALOR := PARAM_VALOR + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'HONORARIOS');
            END IF;
            IF CONTEMINCIDENCIA('JUROS_ORIGINAL', 'MULTA', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_VALOR := PARAM_VALOR + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'JUROS');
            END IF;
            QTDADE_DIAS_ATRASO := ROUND(PARAM_DATA_OPERACAO - PARAM_DATA_VENCIMENTO);
            IF PARAM_IS_DIVIDA_ATIVA = 0
            THEN
                SELECT VALOR
                ...
                WHERE QNTDIAS >= QTDADE_DIAS_ATRASO
                      ...
                      AND ROWNUM = 1
                ORDER BY QNTDIAS;
                IF ITEM_CONFIGURACAO_VALOR IS NOT NULL
                THEN
    ...
    IS
        ...
        BEGIN
            ...
            WHERE spvd.id = getultimasituacao(pvd.id)
                  AND pvd.id = PARAM_ID_PARCELA;
            IF COTA_UNICA = 1 AND PARAM_DATA_OPERACAO < PARAM_VENCIMENTO
            THEN
                RETURN 0;
            END IF;
            IF PARAM_DATA_OPERACAO < PARAM_VENCIMENTO
            THEN
                ----------------------------------------------------------------------------------------------
                ...
                THEN
                    ...
                    IF PAGTO_PARCELAMENTO >= (JUROS + MULTA)
                    THEN
                        PAGTO_PARCELAMENTO := PAGTO_PARCELAMENTO - (JUROS + MULTA);
                        IF PAGTO_PARCELAMENTO >= VALORCORRIGIDO
                        THEN
                            VALORCORRIGIDO := 0;
                        ELSE
            ...
            IF PARAM_SITUACAO = 'PAGO_PARCELAMENTO'
            THEN
                SELECT COALESCE(SUM(PAGAMENTO.CORRECAO),0)
                INTO VALORCORRIGIDO
                FROM PAGAMENTOPORMOVTRIBUTARIA PAGAMENTO
                WHERE parcela_id = PARAM_ID_PARCELA;
            ...
            END IF;
            SELECT
                COALESCE(MAX(PRO.ISENTACORRECAO), 0),
                MAX(PRO.DATAFIM)
            INTO ISENTA_CORRECAO, DATA_ISENCAO_CORRECAO
            FROM ISENCAOACRESCIMOPARCELA ISP
                INNER JOIN PROCESSOISENCAOACRESCIMOS PRO ON PRO.ID = ISP.PROCESSOISENCAOACRESCIMOS_ID
            WHERE ISP.PARCELA_ID = PARAM_ID_PARCELA AND PRO.SITUACAO = 'ATIVO'
                  AND TRUNC(PARAM_DATA_OPERACAO) >= PRO.DATAINICIO;
            IF ISENTA_CORRECAO = 1 AND TRUNC(PARAM_DATA_OPERACAO) <= DATA_ISENCAO_CORRECAO
            THEN
                RETURN 0;
            END IF;
            IF ISENTA_CORRECAO = 1 AND TRUNC(PARAM_DATA_OPERACAO) > DATA_ISENCAO_CORRECAO
            ...
            WHERE DA.DIVIDA_ID = PARAM_DIVIDA_ID
                  ...
                  AND (DA.FINALVIGENCIA IS NULL
                       OR DA.FINALVIGENCIA >= PARAM_DATA_OPERACAO);
            IF CONFIGURACAO_ACRESCIMOS.ID = NULL
            THEN
                RETURN 0;
            END IF;
            IF CONFIGURACAO_ACRESCIMOS.CALCULACORRECAO <> 1
            THEN
                RETURN 0;
            END IF;
            IF CONTEMINCIDENCIA('IMPOSTO', 'CORRECAO', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_SALDO := PARAM_SALDO + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'IMPOSTO');
            END IF;
            IF CONTEMINCIDENCIA('TAXA', 'CORRECAO', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_SALDO := PARAM_SALDO + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'TAXA');
            END IF;
            IF CONTEMINCIDENCIA('MULTA_ORIGINAL', 'CORRECAO', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_SALDO := PARAM_SALDO + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'MULTA');
            END IF;
            IF CONTEMINCIDENCIA('COREECAO_ORIGINAL', 'CORRECAO', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_SALDO := PARAM_SALDO + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'CORRECAO');
            END IF;
            IF CONTEMINCIDENCIA('HONORARIOS_ORIGINAL', 'CORRECAO', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
                PARAM_SALDO := PARAM_SALDO + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'HONORARIOS');
            END IF;
            IF CONTEMINCIDENCIA('JUROS_ORIGINAL', 'CORRECAO', CONFIGURACAO_ACRESCIMOS.ID) = 1
            THEN
            ...
            INTO VALORCORRIGIDO
            FROM DUAL;
            VALORCORRIGIDO := ROUND((VALORCORRIGIDO - PARAM_SALDO) + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'CORRECAO'),
                                    2);
            IF CALCULA_DESCONTO = 1
            THEN
                PAGTO_PARCELAMENTO := PGTOPARCCANCPARCELAMENTO(PARAM_ID_PARCELA);
    ...
    AS
        ...
        BEGIN
            ...
            IF SITUACAO_PARCELA = 'PAGO_PARCELAMENTO' OR SITUACAO_PARCELA = 'PARCELADO'
            THEN
            ...
            END IF;
            IF SITUACAO_PARCELA = 'PAGO' OR SITUACAO_PARCELA = 'BAIXADO'
            THEN
    ...
    AS
        ...
        BEGIN
            SELECT
                COALESCE(pvd.valor, 0)    AS parcelaValor,
    ...
    AS
        ...
        BEGIN
            ...
            SELECT
                COALESCE(pvd.dividaativaajuizada, 0),
                vd.divida_id,
                spvd.situacaoparcela,
                pvd.vencimento
            ...
            IF PARAM_SITUACAO = 'PAGO_PARCELAMENTO'
            THEN
                SELECT COALESCE(SUM(PAGAMENTO.HONORARIOS),0)
                INTO VALOR_HONORARIO
                FROM PAGAMENTOPORMOVTRIBUTARIA PAGAMENTO
                WHERE parcela_id = PARAM_ID_PARCELA;
                IF VALOR_HONORARIO = 0 THEN
    ...
    WHERE IPP.ID =
      ...
      END IF;
            ...
            IF VALOR_EMBUTIDO > VALOR_HONORARIO
            THEN
    ...
    AS
        ...
        BEGIN
            IF PARAM_SITUACAO_PARCELA != 'PAGO' AND PARAM_SITUACAO_PARCELA != 'BAIXADO' AND
               ...
               PARAM_SITUACAO_PARCELA != 'PAGO_SUBVENCAO' AND PARAM_SITUACAO_PARCELA != 'SUBSTITUIDA_POR_COMPENSACAO'
               AND PARAM_SITUACAO_PARCELA != 'COMPENSACAO'
            THEN
                RETURN 0;
            END IF;
            IF PARAM_SITUACAO_PARCELA = 'PAGO_SUBVENCAO'
            THEN
                SELECT SUB.TOTAL
                INTO VALOR
                FROM SUBVENCAOPARCELA SUB
                ...
                RETURN VALOR;
            END IF;
            IF PARAM_SITUACAO_PARCELA = 'SUBSTITUIDA_POR_COMPENSACAO'
            THEN
                ...
                RETURN VALOR;
            END IF;
            IF PARAM_SITUACAO_PARCELA = 'COMPENSACAO'
            THEN
                SELECT PAGAMENTO.VALORCOMPENSADO
                INTO VALOR
                FROM PAGAMENTOJUDICIALPARCELA PAGAMENTO
                ...
                RETURN VALOR;
            END IF;
            IF PARAM_SITUACAO_PARCELA = 'PAGO_PARCELAMENTO'
            THEN
                ...
                WHERE pgto.parcela_id = PARAM_ID_PARCELA
                 ...
                 IF VALOR = 0 THEN
                      SELECT
                      coalesce(SUM(PAGAMENTO.IMPOSTO),0) +
                       coalesce(SUM(PAGAMENTO.TAXA),0) +
                        coalesce(SUM(PAGAMENTO.JUROS),0) +
                         coalesce(SUM(PAGAMENTO.MULTA),0) +
                          coalesce(SUM(PAGAMENTO.CORRECAO),0) +
                           coalesce(SUM(PAGAMENTO.HONORARIOS),0)
                      INTO VALOR
                      FROM PAGAMENTOPORMOVTRIBUTARIA PAGAMENTO
                      WHERE parcela_id = PARAM_ID_PARCELA;
                      END IF;
            ...
            FROM ITEMLOTEBAIXA
                ...
                LEFT JOIN ITEMLOTEBAIXAPARCELA it
                    ON itemlotebaixa.id = it.ITEMLOTEBAIXA_ID AND it.itemdam_id = itemdam.id
            WHERE (LOTEBAIXA.SITUACAOLOTEBAIXA = 'BAIXADO'
                   OR LOTEBAIXA.SITUACAOLOTEBAIXA = 'BAIXADO_INCONSITENTE')
    ...
    AS
        ...
        BEGIN
            IF PARAM_SITUACAO_PARCELA != 'PAGO' AND PARAM_SITUACAO_PARCELA != 'BAIXADO' AND
               PARAM_SITUACAO_PARCELA != 'PAGO_REFIS' AND PARAM_SITUACAO_PARCELA != 'PAGO_PARCELAMENTO' AND
               PARAM_SITUACAO_PARCELA != 'PAGO_SUBVENCAO'
            THEN
                RETURN NULL;
            END IF;
            IF PARAM_SITUACAO_PARCELA = 'PAGO_SUBVENCAO'
    ...
    AS
        ...
        BEGIN
            ...
            FROM SITUACAOPARCELAVALORDIVIDA SPVD
            WHERE SPVD.id = getultimasituacao(PARAM_ID_PARCELA);
            IF SITUACAO_PARCELA = 'PAGO_PARCELAMENTO'
            THEN
                SELECT COALESCE(SUM(PAGAMENTO.IMPOSTO),0)
                INTO RETORNO
                FROM PAGAMENTOPORMOVTRIBUTARIA PAGAMENTO
                WHERE parcela_id = PARAM_ID_PARCELA;
                IF RETORNO = 0 THEN
            ...
            THEN
                ...
                IF PAGTO_PARCELAMENTO >= (JUROS + MULTA + CORRECAO)
                THEN
                    PAGTO_PARCELAMENTO := PAGTO_PARCELAMENTO - (JUROS + MULTA + CORRECAO);
                    IF PAGTO_PARCELAMENTO >= RETORNO
                    THEN
                        RETORNO := 0;
                    ELSE
    ...
    AS
        ...
        BEGIN
            ...
            IF SITUACAO_PARCELA = 'PAGO_PARCELAMENTO'
            THEN
                SELECT COALESCE(SUM(PAGAMENTO.TAXA),0)
                INTO RETORNO
                FROM PAGAMENTOPORMOVTRIBUTARIA PAGAMENTO
                WHERE parcela_id = PARAM_ID_PARCELA;
                IF RETORNO = 0 THEN
            ...
            THEN
                ...
                IF PAGTO_PARCELAMENTO >= (JUROS + MULTA + CORRECAO + IMPOSTO)
                THEN
