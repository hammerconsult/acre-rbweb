  ...
  CREATE OR REPLACE FORCE VIEW VWHIERARQUIAORCAMENTARIA ("ID", "SUPERIOR_ID", "SUBORDINADA_ID", "ENTIDADE_ID", "CODIGO", "DESCRICAO", "EXERCICIO_ID", "TIPOUNIDADE", "TIPOADMINISTRACAO","ESFERADOPODER", "INICIOVIGENCIA", "FIMVIGENCIA", "ANO") AS 
  with rec (id,superior_id, subordinada_id, entidade_id, codigo, descricao, exercicio_id, TIPOUNIDADE, TIPOADMINISTRACAO,esferadopoder, iniciovigencia, fimvigencia, ano ) as (
        select 
               raiz.id,
               raiz.superior_id, 
               raiz.subordinada_id, 
...
) select "ID","SUPERIOR_ID","SUBORDINADA_ID","ENTIDADE_ID","CODIGO","DESCRICAO","EXERCICIO_ID","TIPOUNIDADE","TIPOADMINISTRACAO","ESFERADOPODER","INICIOVIGENCIA","FIMVIGENCIA","ANO" from rec;
