  ...
  select ci.id cadastroimobiliario_id,
    ...
    inner join testada testadas on testadas.lote_id    = lote.id
  ...
  where testadas.id = (select max(s_testa.id) from testada s_testa where s_testa.lote_id = lote.id and s_testa.principal = 1))