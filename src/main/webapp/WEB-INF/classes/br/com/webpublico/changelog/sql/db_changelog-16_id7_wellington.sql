...
BEGIN
  ...
  IF PARAM_SITUACAO = 'PAGO' OR PARAM_SITUACAO = 'BAIXADO' THEN
    ...
    AND PVD.ID                         = PARAM_ID_PARCELA;
    IF(VALOR_JUROS IS NULL OR VALOR_JUROS = 0) THEN
      SELECT IDM.JUROS 
      INTO  VALOR_JUROS
      FROM DAM DM 
      INNER JOIN ITEMDAM IDM ON IDM.DAM_ID = DM.ID
...
BEGIN
  ...
  IF PARAM_SITUACAO = 'PAGO' OR PARAM_SITUACAO = 'BAIXADO' THEN
    ...
    OR LOTEBAIXA.SITUACAOLOTEBAIXA     = 'BAIXADO_INCONSITENTE')
    AND PVD.ID                         = PARAM_ID_PARCELA;
    IF(VALOR_MULTA IS NULL OR VALOR_MULTA = 0) THEN
      SELECT IDM.MULTA 
      INTO  VALOR_MULTA
      FROM DAM DM 
      INNER JOIN ITEMDAM IDM ON IDM.DAM_ID = DM.ID
