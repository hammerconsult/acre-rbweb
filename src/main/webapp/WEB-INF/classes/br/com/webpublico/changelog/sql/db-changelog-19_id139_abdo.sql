  ...
  AS (
  SELECT COALESCE(CI.ID,
          COALESCE(CE.ID,
            COALESCE(CR.ID,
              COALESCE(PF.ID, PJ.ID)))) ID_CADASTRO,
        COALESCE(CI.INSCRICAOCADASTRAL,
            COALESCE(CE.INSCRICAOCADASTRAL,
              COALESCE(CR.NUMEROINCRA,
                COALESCE(PF.CPF, PJ.CNPJ)))) INSCRICAOCADASTRAL,
        COALESCE(PF.CPF, PJ.CNPJ) CPFCNPJ,
        COALESCE(PF.NOME, PJ.RAZAOSOCIAL) NOMERAZAO,
        D.ID ID_DIVIDA,
        D.DESCRICAO DIVIDA,
        CASE WHEN CDA.ID IS NOT NULL THEN CDA.NUMERO||'/'||EXCDA.ANO END NUMEROCERTIDAO,
