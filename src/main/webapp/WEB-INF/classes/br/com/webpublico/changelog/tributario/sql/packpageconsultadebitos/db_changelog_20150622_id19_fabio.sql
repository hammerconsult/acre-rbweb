...
BEGIN
  ...
  WHERE ISP.PARCELA_ID = PARAM_ID_PARCELA AND PRO.SITUACAO = 'ATIVO'
    AND TRUNC(PARAM_DATA_OPERACAO) BETWEEN PRO.DATAINICIO AND DATAFIM;
  IF ISENTA_JUROS = 1 THEN
     RETURN 0;
  END IF;  
  SELECT CA.*
...
BEGIN
  ...
  END IF;
  SELECT COALESCE(MAX(PRO.ISENTAHONORARIOS),0) 
  INTO ISENTA_HONORARIOS
  FROM ISENCAOACRESCIMOPARCELA ISP
  INNER JOIN PROCESSOISENCAOACRESCIMOS PRO ON PRO.ID = ISP.PROCESSOISENCAOACRESCIMOS_ID
  WHERE ISP.PARCELA_ID = PARAM_ID_PARCELA AND PRO.SITUACAO = 'ATIVO'
    AND TRUNC(PARAM_DATA_OPERACAO) BETWEEN PRO.DATAINICIO AND DATAFIM;
  IF ISENTA_HONORARIOS = 1 THEN
     RETURN 0;
  END IF; 
  SELECT CA.*
  INTO CONFIGURACAO_ACRESCIMOS
