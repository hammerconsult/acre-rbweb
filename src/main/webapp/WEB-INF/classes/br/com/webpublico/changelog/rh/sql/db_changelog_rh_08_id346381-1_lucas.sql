...
valor_desc_matric_1 = valor_desc_matric_1 + calculador.valorTotalEventoOutrosVinculos(''908'');

valor_base_inss = valor_desc_matric_1 > 0 ? calculador.calculaBaseMultiplosVinculos(''1018'') : calculador.calculaBase(''1018'');
...
if (calculador.obterCargo(''2208'')) {
    ...
    for (var i = 0; i < faixas.length; i++) {
        ...
        if (faixa.referenciaAte <= valor_base_inss) {
            var valorAcumular = calculador.truncarResultado(Number(ultimaFaixa ? +((faixa.referenciaAte - ultimaFaixa.referenciaAte).toFixed(2)) : faixa.referenciaAte), Number(faixa.percentual));

            acumulado = acumulado + valorAcumular;
            acumulado = +(acumulado.toFixed(2));
        } else {
            var valorAcumular = calculador.truncarResultado(Number(ultimaFaixa ? +((valor_base_inss - ultimaFaixa.referenciaAte).toFixed(2)) : valor_base_inss), Number(faixa.percentual));
...
if (parseFloat(desc_inss).toFixed(2) >= teto && valor_desc_matric_1 <= 0) {
    ...
    referencia = 'var valor_base_inss = calculador.calculaBase(''1018'');
...
return Perc_desc;',
    valorbasedecalculo = 'return calculador.calculaBase(''1018'');',
    calculoretroativo = 0
where codigo = '908'
