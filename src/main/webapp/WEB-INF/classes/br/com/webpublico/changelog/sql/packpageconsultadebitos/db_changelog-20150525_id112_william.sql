...
AS
FUNCTION GETPAGAMENTOAVULSO(
    PARAM_PARCELA_ID IN NUMBER,
    TIPO_TRIBUTO VARCHAR2)
  RETURN NUMBER
AS
  RETORNO NUMBER;
BEGIN
 ...
 -- IF RETORNO IS NULL THEN
    ...
    WHERE PGTO.PARCELAVALORDIVIDA_ID= PARAM_PARCELA_ID
    AND COALESCE(PGTO.ATIVO, 0)     = 1
    AND PGTO.TIPOTRIBUTO            = TIPO_TRIBUTO;
 -- END IF;
  RETURN RETORNO;
EXCEPTION
WHEN NO_DATA_FOUND THEN
  RETURN 0;
END GETPAGAMENTOAVULSO;
...
BEGIN
  ...
  IF PARAM_DATA_VENCIMENTO >= TO_DATE(PARAM_DATA_OPERACAO, 'dd/MM/yyyy') THEN
    ...
    IF CALCULA_DESCONTO     = 1 THEN
      PAGTO_PARCELAMENTO   := COALESCE(GETPAGAMENTOAVULSO(PARAM_ID_PARCELA, 'JUROS'),0);
      IF PAGTO_PARCELAMENTO > 0 THEN
...
BEGIN
  ...
  IF PARAM_DATA_VENCIMENTO >= TO_DATE(PARAM_DATA_OPERACAO, 'dd/MM/yyyy') THEN
    ...
    IF CALCULA_DESCONTO     = 1 THEN
      PAGTO_PARCELAMENTO   := COALESCE(GETPAGAMENTOAVULSO(PARAM_ID_PARCELA, 'MULTA'),0);
      IF PAGTO_PARCELAMENTO > 0 THEN
...
BEGIN
  ...
  IF PARAM_DATA_OPERACAO   <= PARAM_VENCIMENTO THEN
    ...
    IF CALCULA_DESCONTO     = 1 THEN
      VALORCORRIGIDO       := GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'CORRECAO');
      PAGTO_PARCELAMENTO   := COALESCE(GETPAGAMENTOAVULSO(PARAM_ID_PARCELA, 'CORRECAO'),0);
      IF PAGTO_PARCELAMENTO > 0 THEN
        VALORCORRIGIDO     := VALORCORRIGIDO - PAGTO_PARCELAMENTO;
        RETURN PAGTO_PARCELAMENTO;
      END IF;
      PAGTO_PARCELAMENTO      := PGTOPARCCANCPARCELAMENTO(PARAM_ID_PARCELA);
      JUROS                   := GETVALORJUROS( PARAM_DATA_OPERACAO, PARAM_ID_PARCELA, 0);
...
BEGIN
  ...
  IF SITUACAO_PARCELA = 'PAGO' OR SITUACAO_PARCELA = 'BAIXADO' THEN
    ...
    WHERE IDM.ID  =
      ...
      ON S_DAM.ID          = S_IDM.DAM_ID
      WHERE S_DAM.SITUACAO = 'PAGO'
      AND S_IDM.PARCELA_ID = PARAM_PARCELA_ID
      );
...
BEGIN
  ...
  IF PARAM_IS_DIVIDA_AJUIZADA <> 1 THEN
    VALOR_HONORARIO           := 0;
  ELSE
    PARAM_VALOR         := ROUND(GETVALORIMPOSTO(PARAM_PARCELA_ID,1, PARAM_DATA_OPERACAO), 2) + ROUND(GETVALORTAXA(PARAM_PARCELA_ID,1, PARAM_DATA_OPERACAO), 2);
    PARAM_VALOR         := PARAM_VALOR                                                        + ROUND(GETVALORJUROS(PARAM_DATA_OPERACAO, PARAM_PARCELA_ID,1), 2) + ROUND( GETVALORMULTA(PARAM_DATA_OPERACAO,PARAM_PARCELA_ID,1), 2);
    PARAM_VALOR         := PARAM_VALOR                                                        + ROUND(GETVALORCORRECAO(PARAM_PARCELA_ID,EXTRACT(YEAR FROM PARAM_PARCELA_VENCIMENTO),1, PARAM_DATA_OPERACAO), 2);
    PARAM_VALOR         := PARAM_VALOR                                                        - COALESCE(GETVALORDESCONTO(PARAM_PARCELA_ID, PARAM_DATA_OPERACAO),0);
...
AS
  VALOR NUMBER;
BEGIN
  IF PARAM_SITUACAO_PARCELA != 'PAGO' AND PARAM_SITUACAO_PARCELA != 'BAIXADO' AND PARAM_SITUACAO_PARCELA != 'PAGO_REFIS' AND PARAM_SITUACAO_PARCELA != 'PAGO_PARCELAMENTO' AND PARAM_SITUACAO_PARCELA != 'PAGO_SUBVENCAO' AND PARAM_SITUACAO_PARCELA != 'COMPENSACAO' THEN
    RETURN 0;
  END IF;
  ...
  IF PARAM_SITUACAO_PARCELA = 'COMPENSACAO' THEN
    SELECT PARCELA.TOTAL
    INTO VALOR
    FROM PAGAMENTOJUDICIALPARCELA PARCELA
    WHERE PARCELA.ID =
      (SELECT MIN(id)
      FROM PAGAMENTOJUDICIALPARCELA
      WHERE parcelavalordivida_id = PARAM_ID_PARCELA
...
BEGIN
  ...
  IF SITUACAO_PARCELA = 'PAGO_PARCELAMENTO' THEN
    ...
    WHERE IPP.ID =
      (SELECT MAX(id)
      FROM itemprocessoparcelamento
      WHERE parcelavalordivida_id = PARAM_PARCELA_ID
      );
  ...
  IF SITUACAO_PARCELA = 'PARCELADO' THEN
    ...
    RETURN ROUND(RETORNO,2);
  END IF;
  IF SITUACAO_PARCELA = 'COMPENSACAO' THEN
    SELECT PAGAMENTO.IMPOSTO
    INTO RETORNO
    FROM PAGAMENTOJUDICIALPARCELA PAGAMENTO
    WHERE PAGAMENTO.ID =
      (SELECT MIN(id)
  ...
  END IF;
  IF CALCULA_DESCONTO     =1 THEN
    PAGTO_PARCELAMENTO   := COALESCE(GETPAGAMENTOAVULSO(PARAM_PARCELA_ID, 'IMPOSTO'),0);
    IF PAGTO_PARCELAMENTO > 0 THEN
      RETORNO            := RETORNO - PAGTO_PARCELAMENTO;
      RETURN RETORNO;
    END IF;
    PAGTO_PARCELAMENTO := PGTOPARCCANCPARCELAMENTO(PARAM_PARCELA_ID);
    JUROS              := GETVALORJUROS( PARAM_DATA_OPERACAO, PARAM_PARCELA_ID, 0);
    MULTA              := GETVALORMULTA( PARAM_DATA_OPERACAO, PARAM_PARCELA_ID, 0);
    SELECT VENCIMENTO
    INTO VENCIMENTO
    FROM PARCELAVALORDIVIDA
    WHERE ID                 = PARAM_PARCELA_ID;
    CORRECAO                := GETVALORCORRECAO(PARAM_PARCELA_ID, EXTRACT(YEAR FROM VENCIMENTO),0, PARAM_DATA_OPERACAO);
    TAXA                    := GETVALORTAXA(PARAM_PARCELA_ID,0, PARAM_DATA_OPERACAO);
    IF PAGTO_PARCELAMENTO   >= (JUROS             + MULTA + CORRECAO + TAXA) THEN
      PAGTO_PARCELAMENTO    := PAGTO_PARCELAMENTO - (JUROS + MULTA + CORRECAO + TAXA);
...
BEGIN
  ...
  END IF;
  IF CALCULA_DESCONTO     =1 THEN
    PAGTO_PARCELAMENTO   := COALESCE(GETPAGAMENTOAVULSO(PARAM_PARCELA_ID, 'TAXA'),0);
    IF PAGTO_PARCELAMENTO > 0 THEN
      RETORNO            := RETORNO - PAGTO_PARCELAMENTO;
      RETURN RETORNO;
    END IF;
