  ...
  SELECT DISTINCT CADASTRO.ID AS CADASTRO_ID,
    ...
    round(PACOTE_CONSULTA_DE_DEBITOS.GETVALORCORRECAO( PVD.ID, EXERCICIO.ANO), 2)                             AS VALORCORRECAO,
    round(PACOTE_CONSULTA_DE_DEBITOS.GETVALORJUROS(SYSDATE, PVD.ID), 2)                                       AS VALORJUROS,
    round(PACOTE_CONSULTA_DE_DEBITOS.GETVALORMULTA(SYSDATE, PVD.ID), 2)                                       AS VALORMULTA,
    round(PACOTE_CONSULTA_DE_DEBITOS.GETVALORHONORARIOS(PVD.ID), 2)                                           AS VALORHONORARIOS,
    round(PACOTE_CONSULTA_DE_DEBITOS.GETVALORIMPOSTO(PVD.VALOR, PVD.ID, SPVD.SALDO), 2)                       AS VALORIMPOSTO,
    round(PACOTE_CONSULTA_DE_DEBITOS.GETVALORTAXA(PVD.VALOR, PVD.ID, SPVD.SALDO), 2)                          AS VALORTAXA,
    round(PACOTE_CONSULTA_DE_DEBITOS.GETVALORDESCONTO(PVD.ID), 2)                                             AS VALORDESCONTO,
