...
BEGIN
  ...
  IF to_date(PARAM_DATA_VENCIMENTO, 'dd/MM/yy') >= to_date(PARAM_DATA_OPERACAO, 'dd/MM/yy') THEN
    RETURN 0;
  END IF;
  ...
  DBMS_APPLICATION_INFO.set_client_info('some information here');
  IF CONFIGURACAO_ACRESCIMOS.ID IS NULL THEN
    RETURN 0;
  END IF;
  IF CONFIGURACAO_ACRESCIMOS.TIPOAPLICACRESCIMOJUROS = 'PRINCIPAL_CORRECAO' THEN
   PARAM_VALOR := PARAM_VALOR + GETVALORCORRECAO(PARAM_ID_PARCELA, PARAM_EXERCICIO);  
  END IF;
  DIFERENCA_DIAS    := to_date(PARAM_DATA_OPERACAO, 'dd/MM/yyyy') - to_date(PARAM_DATA_VENCIMENTO, 'dd/MM/yyyy');
...
BEGIN
  ...
  IF CONFIGURACAO_ACRESCIMOS.ID = NULL THEN
    RETURN 0;
  END IF;
  IF CONFIGURACAO_ACRESCIMOS.TIPOAPLICACRESCIMOMULTA = 'PRINCIPAL_CORRECAO' THEN
   PARAM_VALOR := PARAM_VALOR + GETVALORCORRECAO(PARAM_ID_PARCELA, PARAM_EXERCICIO);  
  END IF;
