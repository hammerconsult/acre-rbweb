 create or replace force view vwconsultafiscsecretaria as
  select distinct
           ...
           us.id id_usuario,
           us.login login_usuario,
           sf.id id_secretariafiscalizacao,
           sf.codigo||' - '||uo.descricao secretaria
       from calculoprocfiscalizacao cpf
      inner join processofiscalizacao pcf on pcf.id = cpf.processofiscalizacao_id
      inner join secretariafiscalizacao sf on sf.id = pcf.secretariafiscalizacao_id
      inner join unidadeorganizacional uo on uo.id = sf.unidadeorganizacional_id
      inner join autoinfracaofiscalizacao aif on aif.id = pcf.autoinfracaofiscalizacao_id
      inner join usuariosistema us on us.id = aif.usuariosistema_id
