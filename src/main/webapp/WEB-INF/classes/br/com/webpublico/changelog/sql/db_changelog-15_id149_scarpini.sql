  ...
  CREATE OR REPLACE FORCE VIEW VWCONSULTADEDEBITOS AS 
  SELECT DISTINCT
  ...
  case WHEN spvd.situacaoParcela = 'PAGO' then pvd.valor else 
  spvd.saldo end          AS VALORORIGINAL,
  case when spvd.situacaoparcela = 'EM_ABERTO' then
  pacote_consulta_de_debitos.getvalorcorrecao(spvd.saldo, pvd.id, pvd.dataregistro, exercicio.ano)
  else 0 end AS VALORCORRECAO,
  case when spvd.situacaoparcela = 'EM_ABERTO' then
  pacote_consulta_de_debitos.getvalorjuros(pvd.vencimento, sysdate, divida.id, spvd.saldo, COALESCE(pvd.dividaativa, 0))  else 0 end  AS VALORJUROS,
  case when spvd.situacaoparcela = 'EM_ABERTO' then 
  pacote_consulta_de_debitos.getvalormulta(pvd.vencimento, sysdate, divida.id, spvd.saldo, COALESCE(pvd.dividaativa, 0))   else 0 end  AS VALORMULTA,
  case when spvd.situacaoparcela = 'EM_ABERTO' then 
  pacote_consulta_de_debitos.getvalorhonorarios(divida.id, spvd.saldo, COALESCE(pvd.dividaAtivaAjuizada, 0))   else 0 end  as VALORHONORARIOS,
