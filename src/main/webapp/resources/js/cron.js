 ...
 * @return string
 */
function cronCalculate(zeroAllowed, max, values, all_selected) {
    if (zeroAllowed == false) values.push(0);
    if (values.length > max || values.length == 0) return all_selected;
    ...
    out:
        for (var d = 2; d <= Math.ceil(max / 2); d++) {
            ...
            for (x = 0; x * d <= max; x++) {
                if (tmp.indexOf(x * d) == -1) continue out; else tmp.splice(tmp.indexOf(x * d), 1);
            }
 ...
 * @return int[]
 */
function cronValueItemToList(allowZero, maxValue, value, all_selected) {
    var list = [];
    if (value == all_selected) {
        ...
        for (var i = 0; i < a.length; i++) {
            var e = a[i].split("-");
            if (e.length == 2) {
                for (var j = parseInt(e[0]); j <= parseInt(e[1]); j++) list.push(j);
            } else {
...
function importCronExpression(expression) {
    if (!expression.match(/^\s*($|#|\w+\s*=|(\?|\*|(?:[0-5]?\d)(?:(?:-|\/|\,)(?:[0-5]?\d))?(?:,(?:[0-5]?\d)(?:(?:-|\/|\,)(?:[0-5]?\d))?)*)\s+(\?|\*|(?:[0-5]?\d)(?:(?:-|\/|\,)(?:[0-5]?\d))?(?:,(?:[0-5]?\d)(?:(?:-|\/|\,)(?:[0-5]?\d))?)*)\s+(\?|\*|(?:[01]?\d|2[0-3])(?:(?:-|\/|\,)(?:[01]?\d|2[0-3]))?(?:,(?:[01]?\d|2[0-3])(?:(?:-|\/|\,)(?:[01]?\d|2[0-3]))?)*)\s+(\?|\*|(?:0?[1-9]|[12]\d|3[01])(?:(?:-|\/|\,)(?:0?[1-9]|[12]\d|3[01]))?(?:,(?:0?[1-9]|[12]\d|3[01])(?:(?:-|\/|\,)(?:0?[1-9]|[12]\d|3[01]))?)*)\s+(\?|\*|(?:[1-9]|1[012])(?:(?:-|\/|\,)(?:[1-9]|1[012]))?(?:L|W)?(?:,(?:[1-9]|1[012])(?:(?:-|\/|\,)(?:[1-9]|1[012]))?(?:L|W)?)*|\?|\*|(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)(?:(?:-)(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC))?(?:,(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)(?:(?:-)(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC))?)*)\s+(\?|\*|(?:[0-6])(?:(?:-|\/|\,|#)(?:[0-6]))?(?:L)?(?:,(?:[0-6])(?:(?:-|\/|\,|#)(?:[0-6]))?(?:L)?)*|\?|\*|(?:MON|TUE|WED|THU|FRI|SAT|SUN)(?:(?:-)(?:MON|TUE|WED|THU|FRI|SAT|SUN))?(?:,(?:MON|TUE|WED|THU|FRI|SAT|SUN)(?:(?:-)(?:MON|TUE|WED|THU|FRI|SAT|SUN))?)*)(|\s)+(\?|\*|(?:|\d{4})(?:(?:-|\/|\,)(?:|\d{4}))?(?:,(?:|\d{4})(?:(?:-|\/|\,)(?:|\d{4}))?)*))$/)) {
        console.log("expression invalid");
        return;
    }
    var parts = expression.split(" ");
    var tmp;
    if (parts[1] != cron_minutes) {
        cron_minutes = parts[1];
        cronHelperSelectList(cron_minutes_id, cronValueItemToList(true, 59, parts[1], "*"));
    }
    if (parts[2] != cron_hours) {
        cron_hours = parts[2];
        cronHelperSelectList(cron_hours_id, cronValueItemToList(true, 23, parts[2], "*"));
    }
    if (parts[3] != cron_dom) {
        cron_dom = parts[3];
        cronHelperSelectList(cron_dom_id, cronValueItemToList(false, 31, parts[3], "?"));
    }
    if (parts[4] != cron_months) {
        cron_months = parts[4];
        cronHelperSelectList(cron_months_id, cronValueItemToList(false, 12, parts[4], "*"));
    }
    if (parts[4] != cron_dow) {
        cron_dow = parts[5];
        cronHelperSelectList(cron_dow_id, cronValueItemToList(true, 6, parts[5], "*"));
    }
} // }}}
...
function getCronExpression() {
    return "0 " + cron_minutes + " " + cron_hours + " " + cron_dom + " " + cron_months + " " + cron_dow;
} // }}}
...
function getSelectedElements(id) {
    return $.map($(id + ' > option:selected'), function (element) {
        if (parseInt(element.value) != "NaN") return parseInt(element.value);
    });
} // }}}
