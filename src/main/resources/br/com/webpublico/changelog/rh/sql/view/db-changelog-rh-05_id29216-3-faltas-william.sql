CREATE OR REPLACE FORCE VIEW VWFALTA as
SELECT
            falta.contratoFP_id,
            falta.falta_id,
    ...
    FROM (
      ...
      (select fim - inicio from faltas faltasub where faltasub.id = f.id)
     -  coalesce((select sum (SUBJUSTIFICATIVA.FINALVIGENCIA - SUBJUSTIFICATIVA.iniciovigencia) + count(SUBJUSTIFICATIVA.id) -1 from justificativafaltas SUBJUSTIFICATIVA
      where SUBJUSTIFICATIVA.FALTAS_ID = F.ID), 0)
      as totalFaltasEfetivas,
      ...
      f.DATACADASTRO as datacadastro
         from faltas f left join JUSTIFICATIVAFALTAS jf on jf.FALTAS_ID = f.id
         ) falta
