update itemprocesso set query35 = 'select COALESCE(COALESCE(SUM(saldo.credito) - SUM(COALESCE(saldoInicial.credito, 0)), 0) - COALESCE(SUM(saldo.debito) - SUM(COALESCE(saldoInicial.debito, 0)) - SUM(COALESCE(saldoInicial.credito, 0) - COALESCE(saldoInicial.debito, 0)), 0), 0) as saldofinal
from (
       ...
       from SaldoGrupoBem mov
         inner join GRUPOBEM gb on gb.ID = mov.GRUPOBEM_ID and gb.TIPOBEM = ''MOVEIS''
         inner join CONFIGCONCCONTABILGRUPBEMM cfg on gb.ID = cfg.GRUPOBEM_ID
       where trunc(mov.datasaldo) = (
         select max(trunc(saldo.datasaldo)) from SaldoGrupoBem saldo
  ...
  left join (select sld.GRUPOBEM_ID as grupoid,
             ...
             from SaldoGrupoBem sld
               inner join GRUPOBEM gb on gb.ID = sld.GRUPOBEM_ID and gb.TIPOBEM = ''MOVEIS''
               inner join CONFIGCONCCONTABILGRUPBEMM cf on gb.ID = cf.GRUPOBEM_ID
