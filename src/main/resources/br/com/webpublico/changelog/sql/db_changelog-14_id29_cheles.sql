...
SELECT DISTINCT
  ...
  pacote_consulta_de_debitos.getvalorcorrigido(spvd.saldo, pvd.id,
  pvd.dataregistro, exercicio.ano) AS VALORCORRIGIDO,
  pacote_consulta_de_debitos.getvalorjuros(pvd.vencimento, sysdate, divida.id,
  pacote_consulta_de_debitos.getvalorcorrigido(spvd.saldo, pvd.id,
  pvd.dataregistro, exercicio.ano), COALESCE(pvd.dividaativa, 0)) AS VALORJUROS
  ,
  pacote_consulta_de_debitos.getvalormulta(pvd.vencimento, sysdate, divida.id,
  pacote_consulta_de_debitos.getvalorcorrigido(spvd.saldo, pvd.id,
  pvd.dataregistro, exercicio.ano), COALESCE(pvd.dividaativa, 0)) AS VALORMULTA
  ,
  pacote_consulta_de_debitos.getvalorimposto(pvd.valor, pvd.id, spvd.saldo) AS
  VALORIMPOSTO,
  pacote_consulta_de_debitos.getvalortaxa(pvd.valor, pvd.id, spvd.saldo) AS
  VALORTAXA,
