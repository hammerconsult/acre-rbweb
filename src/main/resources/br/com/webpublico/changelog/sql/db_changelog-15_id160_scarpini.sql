  ...
  case WHEN spvd.situacaoParcela <> 'EM_ABERTO' then pvd.valor else 
  spvd.saldo end          AS VALORORIGINAL,
  pacote_consulta_de_debitos.getvalorcorrecao( pvd.id, exercicio.ano) AS VALORCORRECAO,
  pacote_consulta_de_debitos.getvalorjuros(sysdate, pvd.id) AS VALORJUROS,
  pacote_consulta_de_debitos.getvalormulta(sysdate, pvd.id)  AS VALORMULTA,
  pacote_consulta_de_debitos.getvalorhonorarios(pvd.id) as VALORHONORARIOS,
