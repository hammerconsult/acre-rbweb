...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE(PARAM_DATA_VENCIMENTO || '  ' || PARAM_DATA_OPERACAO);
  ...
  IF PARAM_SITUACAO = 'PAGO' OR PARAM_SITUACAO = 'BAIXADO' THEN
    ...
    ON IDM.DAM_ID        = DM.ID
    WHERE IDM.PARCELA_ID = PARAM_ID_PARCELA
    AND DM.SITUACAO      = 'PAGO';
  ...
  END IF;
   IF PARAM_SITUACAO = 'PARCELADO' THEN
    SELECT SUM(IPP.JUROS)
      ...
      and pp.situacaoParcelamento = 'FINALIZADO';
      RETURN ROUND(VALOR_JUROS,2);
   END IF;
  ...
  END IF;
  VALOR_JUROS             := ROUND(VALOR_JUROS + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'JUROS'),2);
...
BEGIN
  ...
  VALOR_MULTA               := ROUND(VALOR_MULTA + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'MULTA'),2);
  IF CALCULA_DESCONTO        = 1 THEN
...
BEGIN
  ...
  IF COTA_UNICA = 1 AND sysdate < PARAM_VENCIMENTO THEN
    RETURN 0;
  END IF;
  IF sysdate < PARAM_VENCIMENTO THEN
...
BEGIN
  ...
  SELECT SUM(COALESCE(desconto.desconto,0))
  INTO DESCONTO_VALOR
  FROM descontoitemparcela desconto
  INNER JOIN itemparcelavalordivida item
  ON item.id                           = desconto.itemparcelavalordivida_id
...
BEGIN
  ...
  IF PARAM_SITUACAO = 'PARCELADO' THEN
    SELECT SUM(IPP.HONORARIOS)
      ...
      inner join processoparcelamento pp on pp.id = ipp.processoparcelamento_id
      WHERE IPP.PARCELAVALORDIVIDA_ID = PARAM_ID_PARCELA
      and pp.situacaoParcelamento = 'FINALIZADO';      RETURN ROUND(VALOR_HONORARIO,2);
   END IF;
  IF PARAM_SITUACAO <> 'EM_ABERTO' THEN
    RETURN 0;
  END IF;
  ...
  ELSE
    ...
    PARAM_VALOR         := PARAM_VALOR                                   + ROUND(GETVALORJUROS(SYSDATE, PARAM_PARCELA_ID,1), 2) + ROUND( GETVALORMULTA(SYSDATE,PARAM_PARCELA_ID,1), 2);
    PARAM_VALOR         := PARAM_VALOR                                   + ROUND(GETVALORCORRECAO(PARAM_PARCELA_ID,extract(YEAR FROM PARAM_PARCELA_VENCIMENTO),1), 2);
    PORCENTAGEM_APLICAR := CONFIGURACAO_ACRESCIMOS.HONORARIOSADVOCATICIOS;
    VALOR_HONORARIO     := (PORCENTAGEM_APLICAR * PARAM_VALOR) / 100;
  END IF;
  RETURN ROUND(VALOR_HONORARIO,2) + GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'HONORARIOS');
...
AS
  VALOR NUMBER;
BEGIN
  IF PARAM_SITUACAO_PARCELA != 'PAGO' AND PARAM_SITUACAO_PARCELA != 'BAIXADO' AND PARAM_SITUACAO_PARCELA != 'PAGO_REFIS' THEN
    RETURN 0;
  END IF;
...
BEGIN
  SELECT SUM(valor)
  INTO RETORNO
  FROM PGTOPARCCANCPARCELAMENTO PGTO
...
BEGIN
  ...
  IF CALCULA_DESCONTO        =1 THEN
    ...
    SELECT VENCIMENTO INTO VENCIMENTO FROM PARCELAVALORDIVIDA WHERE ID = PARAM_PARCELA_ID;
    CORRECAO                := GETVALORCORRECAO(PARAM_PARCELA_ID, EXTRACT(YEAR FROM VENCIMENTO),0);
