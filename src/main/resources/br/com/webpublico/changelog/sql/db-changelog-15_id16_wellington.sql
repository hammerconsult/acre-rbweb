CREATE OR REPLACE FORCE VIEW "VWCONSULTADEDEBITOS" ("CADASTRO_ID", "PESSOA_ID", "DIVIDA_ID", "CALCULO_ID", "VALORDIVIDA_ID", "PARCELA_ID", "SITUACAOPARCELA_ID", "SITUACAOPARCELA", "EXERCICIO_ID", "OPCAOPAGAMENTO_ID", "VALORORIGINAL", "VALORCORRECAO", "VALORJUROS", "VALORMULTA", "VALORHONORARIOS", "VALORIMPOSTO", "VALORTAXA", "VALORDESCONTO", "PARCELA", "PROMOCIONAL", "VALORPAGO", "DATAPAGAMENTO", "EMISSAO", "PROCESSOCALCULO")
                              AS
  ...
  ON SPVD.ID = (SELECT MAX(ID) FROM SITUACAOPARCELAVALORDIVIDA WHERE PARCELA_ID = PVD.ID)
  ...
  ( ( OP.PROMOCIONAL = 1
   ...
   OR (OP.PROMOCIONAL = 1 AND PVD.VENCIMENTO <= SYSDATE AND SPVD.SITUACAOPARCELA <> 'EM_ABERTO')
   OR ( COALESCE(OP.PROMOCIONAL, 0) <> 1 ) )