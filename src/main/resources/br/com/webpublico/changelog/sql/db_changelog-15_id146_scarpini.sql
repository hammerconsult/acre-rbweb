...
BEGIN
  ...
  IF CONFIGURACAO_ACRESCIMOS.TIPOAPLICACRESCIMOMULTA   = 'PRINCIPAL_JUROS' THEN
    IF CONFIGURACAO_ACRESCIMOS.TIPOAPLICACRESCIMOJUROS = 'PRINCIPAL_MULTA' THEN
      RAISE TIPO_APLIC_ACRESCIMO_EXCEPTION;
    END IF;
    VALOR_MULTA := VALOR_MULTA + PACOTE_CONSULTA_DE_DEBITOS.GETVALORJUROS(PARAM_DATA_VENCIMENTO, PARAM_DATA_OPERACAO, PARAM_DIVIDA_ID, PARAM_VALOR, PARAM_IS_DIVIDA_ATIVA);
  END IF;
  RETURN VALOR_MULTA;
EXCEPTION
WHEN NO_DATA_FOUND THEN
...
inner join ITEMLOTEBAIXA on itemlotebaixa.id = it.ITEMLOTEBAIXA_ID
inner join LOTEBAIXA on lotebaixa.id = ITEMLOTEBAIXA.LOTEBAIXA_ID
inner join itemdam on itemdam.id = it.ITEMDAM_ID
inner join parcelavalordivida pvd on pvd.id= itemdam.PARCELA_ID
where (LOTEBAIXA.SITUACAOLOTEBAIXA = 'BAIXADO' or LOTEBAIXA.SITUACAOLOTEBAIXA = 'BAIXADO_INCONSITENTE') and
PVD.ID = PARAM_ID_PARCELA;
RETURN VALOR;
