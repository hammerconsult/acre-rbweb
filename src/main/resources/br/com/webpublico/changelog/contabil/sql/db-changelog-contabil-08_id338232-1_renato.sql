CREATE OR REPLACE FORCE VIEW VWHIERARQUIADESPESAORC (ID, ORGAO, UNIDADE, SUBACAO, CONTA, EXERCICIO, UNIDADE_ID, TIPOCONTADESPESA, INICIOVIGENCIA, FIMVIGENCIA, CODIGOREDUZIDO, conta_id) AS
SELECT D.ID AS id,
       hoUo.CODIGO
           ||' - '
       ...
       con.id              AS CONTA_ID
FROM DESPESAORC D
         inner join exercicio ex on d.EXERCICIO_ID = ex.id
    ...
    and to_date('01/01/'||ex.ano, 'dd/MM/yyyy') between HOUO.INICIOVIGENCIA and coalesce(HOUO.FIMVIGENCIA, to_date('01/01/'||ex.ano, 'dd/MM/yyyy'))
