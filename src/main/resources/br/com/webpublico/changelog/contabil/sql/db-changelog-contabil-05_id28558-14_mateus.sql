merge into SALDODIVIDAATIVACONTABIL s using (
...
and fr.EXERCICIO_ID = (select id from exercicio where ano = extract(year from datasaldo))) as CONTADEDESTINACAO_ID from SALDODIVIDAATIVACONTABIL
) sld on (sld.id = s.id)
