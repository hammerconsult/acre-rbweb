  ...
  SELECT DISTINCT CADASTRO.ID AS CADASTRO_ID,
   ...
   CASE WHEN CALCULO.TIPOCALCULO = 'PARCELAMENTO' THEN
      ROUND(PACOTE_CONSULTA_DE_DEBITOS.GETVALORCORRECAO(PVD.ID, EXTRACT(YEAR FROM vd.emissao),1, SYSDATE), 2)
   ELSE
      ROUND(PACOTE_CONSULTA_DE_DEBITOS.GETVALORCORRECAO(PVD.ID, EXTRACT(YEAR FROM pvd.vencimento),1, SYSDATE), 2)
   END
    AS VALORCORRECAO,
