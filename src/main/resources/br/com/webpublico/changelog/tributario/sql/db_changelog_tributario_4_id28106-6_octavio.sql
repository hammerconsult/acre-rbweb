create or replace view vwconsultahabitese as
select distinct
    ci.id id_cadastro,
    ...
    ci.ativo situacao,
    calctd.id id_calculotaxasdiversas,
    c.tipocalculo tipocalculo,
    trib.id id_tributo
from calculotaxasdiversas calctd
         inner join calculo c on c.id = calctd.id
         ...
         inner join cadastroimobiliario ci on ci.id = c.cadastro_id
         ...
         left join itemparcelavalordivida item on pvd.ID = item.parcelavalordivida_id
         left join itemvalordivida itemvd on item.itemvalordivida_id = itemvd.ID
         left join tributo trib on itemvd.tributo_id = trib.ID
