...
BEGIN
  ...
  END IF;
  SELECT COALESCE(MAX(PRO.ISENTAJUROS),0), MAX(PRO.DATAFIM)
...
BEGIN
 ...
 IF PARAM_SITUACAO = 'PAGO' OR PARAM_SITUACAO = 'BAIXADO' THEN
  ...
  IF PARAM_SITUACAO <> 'EM_ABERTO' THEN
    RETURN GETVALORTIPOTRIBUTO(PARAM_ID_PARCELA, 'CORRECAO');
  END IF;
  SELECT COALESCE(MAX(PRO.ISENTACORRECAO),0), MAX(PRO.DATAFIM)
...
BEGIN
  ...
  IF CALCULA_DESCONTO   =1 THEN
    ...
    WHERE ID                 = PARAM_PARCELA_ID;
    CORRECAO                := GETVALORCORRECAO(PARAM_PARCELA_ID, EXTRACT(YEAR FROM VENCIMENTO),0, PARAM_DATA_OPERACAO);
    IMPOSTO                 := GETVALORIMPOSTO(PARAM_PARCELA_ID,0, PARAM_DATA_OPERACAO);
