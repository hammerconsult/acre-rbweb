    ...
    select distinct
       ...
       from calculo c
      left join calculofiscalizacao cf on cf.id = c.id
      left join calculomultafiscalizacao cmf on cmf.id = c.id
      inner join autoinfracaofiscal aif on aif.id = coalesce(cf.autoinfracaofiscal_id, cmf.autoinfracaofiscal_id)
