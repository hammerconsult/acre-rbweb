 ...
 * @author Gustavo
 */
@FacesValidator("validadorCPFCNPJ")
public class ValidaCPFCNPJ implements Validator {

    public void validate(FacesContext context, UIComponent component, Object value) throws ValidatorException {
        FacesMessage message = new FacesMessage();
        String nomeCampo;
        if (String.valueOf(value).length() > 14) {
            nomeCampo = "CNPJ";
        } else {
            nomeCampo = "CPF";
        }
        if (!valida_CpfCnpj(String.valueOf(value))) {
            message.setDetail(nomeCampo + " digitado não é valido");
            message.setSummary(nomeCampo +
                    " digitado não é valido");
