...
@ManagedBean(name = "cargoConfiancaControlador")
@ViewScoped
@URLMappings(mappings = {
    @URLMapping(id = "novoAcessoCargoComissao", pattern = "/acesso-cargo-comissao/novo/", viewId = "/faces/rh/administracaodepagamento/cargoconfianca/edita.xhtml"),
    @URLMapping(id = "editarAcessoCargoComissao", pattern = "/acesso-cargo-comissao/editar/#{cargoConfiancaControlador.id}/", viewId = "/faces/rh/administracaodepagamento/cargoconfianca/edita.xhtml"),
    @URLMapping(id = "listarAcessoCargoComissao", pattern = "/acesso-cargo-comissao/listar/", viewId = "/faces/rh/administracaodepagamento/cargoconfianca/lista.xhtml"),
    @URLMapping(id = "verAcessoCargoComissao", pattern = "/acesso-cargo-comissao/ver/#{cargoConfiancaControlador.id}/", viewId = "/faces/rh/administracaodepagamento/cargoconfianca/visualizar.xhtml")
})
public class CargoConfiancaControlador extends PrettyControlador<CargoConfianca> implements Serializable, CRUD {
    ...
    public CargoConfiancaControlador() {
    ...
    public boolean validaContrato() {
        if ((selecionado).getContratoFP() != null) {
            if (!contratoFPFacade.verificaContratoEnquadramentoFuncional(((CargoConfianca) selecionado).getContratoFP()) && !selecionado.getContratoFP().getTipoRegime().getCodigo().equals(TipoRegime.ESTATUTARIO)) {
                FacesContext.getCurrentInstance().addMessage("Formulario:contratoFP", new FacesMessage(FacesMessage.SEVERITY_ERROR, "Problema", "O Acesso a Cargo em comissão só pode ser feito para servidores concursados e do tipo de regime estatutário."));
                cancelaContratoFP();
                return false;
            }
            ...
            if (!cargoConfiancaFacade.buscarVinculofpPorServidorAndPeriodo(selecionado.getContratoFP(),
                selecionado.getInicioVigencia(), selecionado.getFinalVigencia()).isEmpty()
                && isOperacaoNovo()) {
                FacesUtil.addOperacaoNaoPermitida(" Já existe um registro de Acesso a Cargo em Comissão no mesmo período.");
                cancelaContratoFP();
                return false;
            }
        }
    ...
    private void cancelaContratoFP() {
        ((CargoConfianca) selecionado).setContratoFP(null);
    }

    private boolean servidorEstaEmFuncaoGratificadaVigente() {
        return cargoConfiancaFacade.getFuncaoGratificadaFacade().recuperaFuncaoGratificada(((CargoConfianca) selecionado).getContratoFP()) != null;
    }
    ...
    public List<Cargo> completarCargos(String parte) {
        return cargoFacade.buscarCargosPorTipoPCSAndVigenteAndAtivoAndCodigoOrDescricaoAndUnidadeOrganizacionalUsuario(parte.trim(), TipoPCS.CARGO_EM_COMISSAO);
    }
    ...
    @URLAction(mappingId = "novoAcessoCargoComissao", phaseId = URLAction.PhaseId.RENDER_RESPONSE, onPostback = false)
    @Override
    public void novo() {
        super.novo();
    ...
    @URLAction(mappingId = "verAcessoCargoComissao", phaseId = URLAction.PhaseId.RENDER_RESPONSE, onPostback = false)
    @Override
    public void ver() {
        super.ver();
        selecionar();
    ...
    @URLAction(mappingId = "editarAcessoCargoComissao", phaseId = URLAction.PhaseId.RENDER_RESPONSE, onPostback = false)
    @Override
    public void editar() {
        super.editar();
        selecionar();
    ...
    public void salvar() {
        ...
        try {
            if (isOperacaoNovo()) {
                ...
                cargoConfiancaFacade.validarValoresEntidade(selecionado);
            }
            ...
            cargoConfiancaFacade.salvarCargoConfianca(selecionado, provimentoFP);
            redireciona();
    ...
    public void confirmarEnquadramentoCC() {
        if (!validarCamposObrigatoriosEnquadramentoCC()) {
            return;
        }
        ...
        enquadramentoCC.setCargoConfianca(selecionado);
    ...
    private boolean validarCamposObrigatoriosEnquadramentoCC() {
        boolean valida = true;
        if (enquadramentoCC.getInicioVigencia() == null) {
            FacesUtil.addCampoObrigatorio("O campo data de início de vigência deve ser informado.");
            valida = false;
        }
        ...
        if (enquadramentoCC.getCategoriaPCS() == null) {
            FacesUtil.addCampoObrigatorio("O campo categoriaPCS deve ser informado.");
            valida = false;
        }
        if (enquadramentoPCS == null || enquadramentoPCS.getVencimentoBase() == null || enquadramentoPCS.getVencimentoBase().compareTo(BigDecimal.ZERO) <= 0) {
            FacesUtil.addOperacaoNaoPermitida("O valor de referência deve ser maior que 0(zero).");
            valida = false;
        }
        return valida;
    }
    ...
    public void validarCampos() {
        ...
        for (EnquadramentoCC enquadramento : selecionado.getEnquadramentoCCs()) {
            if (enquadramento.getFinalVigencia() == null || DataUtil.isVigente(cargoConfiancaFacade.getSistemaFacade().getDataOperacao(), enquadramento.getFinalVigencia())) {
