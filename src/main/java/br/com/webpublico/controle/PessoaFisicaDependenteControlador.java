 ...
 * @author Andre Peres
 */
@ManagedBean(name = "pessoaFisicaDependenteControlador")
@ViewScoped
@URLMappings(mappings = {
    @URLMapping(id = "novoPessoaDependente", pattern = "/pessoa-dependente/novo/", viewId = "/faces/rh/administracaodepagamento/pessoadependente/edita.xhtml"),
    @URLMapping(id = "editarPessoaDependente", pattern = "/pessoa-dependente/editar/#{pessoaFisicaDependenteControlador.id}/", viewId = "/faces/rh/administracaodepagamento/pessoadependente/edita.xhtml"),
    @URLMapping(id = "listarPessoaDependente", pattern = "/pessoa-dependente/listar/", viewId = "/faces/rh/administracaodepagamento/pessoadependente/lista.xhtml"),
    @URLMapping(id = "verPessoaDependente", pattern = "/pessoa-dependente/ver/#{pessoaFisicaDependenteControlador.id}/", viewId = "/faces/rh/administracaodepagamento/pessoadependente/visualizar.xhtml")
})
public class PessoaFisicaDependenteControlador extends PrettyControlador<PessoaFisica> implements Serializable, CRUD {

    private static final Logger logger = LoggerFactory.getLogger(PessoaFisicaDependenteControlador.class);
    ...
    private PessoaFisicaDependenteControlador pessoaFisicaDependenteControlador = this;
    private boolean skip;
    @EJB
    private DocumentoPessoalFacade documentoPessoalFacade;
    private PessoaFisicaCid pessoaFisicaCid;

    public PessoaFisicaDependenteControlador() {
        super(PessoaFisica.class);
    }

    @URLAction(mappingId = "novoPessoaDependente", phaseId = URLAction.PhaseId.RENDER_RESPONSE, onPostback = false)
    public void nova() {
        novoFisico();
    }

    @URLAction(mappingId = "editarPessoaDependente", phaseId = URLAction.PhaseId.RENDER_RESPONSE, onPostback = false)
    @Override
    public void editar() {
        selecionarPessoa();
    ...
    @URLAction(mappingId = "verPessoaDependente", phaseId = URLAction.PhaseId.RENDER_RESPONSE, onPostback = false)
    @Override
    public void ver() {
        selecionarPessoa();
    ...
    public void carregarDaSessao() {
        PessoaFisicaDependenteControlador p = (PessoaFisicaDependenteControlador) Web.pegaDaSessao(PessoaFisicaDependenteControlador.class);
        if (!isSessao() || p == null) {
            return;
        }
        pessoaFisicaDependenteControlador = p;
        definirAtributos(p);
    }

    public void definirAtributos(PessoaFisicaDependenteControlador controler) {
    ...
    private boolean isCerticaoNascimentoPreenchida() {
        return certidaoNascimento != null
...
//            tituloEleitores.get(0).setPessoaFisica((PessoaFisica) selecionado);
    ...
    public void removeHabilitacao(ActionEvent evento) {
        PessoaFisica pf = selecionado;
        pf.getDocumentosPessoais().remove(evento.getComponent().getAttributes().get("removeHabilitacao"));
        habilitacoes.remove(evento.getComponent().getAttributes().get("removeHabilitacao"));
        habilitacao = new Habilitacao();
    }

    public void removeTrabalho(ActionEvent evento) {
        PessoaFisica pf = selecionado;
        pf.getDocumentosPessoais().remove(evento.getComponent().getAttributes().get("removeTrabalho"));
        carteiraTrabalhos.remove(evento.getComponent().getAttributes().get("removeTrabalho"));
        carteiraTrabalho = new CarteiraTrabalho();
    }
    ...
    public void removeFone(ActionEvent evento) {
        selecionado.getTelefones().remove(evento.getComponent().getAttributes().get("vfone"));
    }

    public void removeEndereco(ActionEvent evento) {
        selecionado.getEnderecos().remove(evento.getComponent().getAttributes().get("vendereco"));
    }
    ...
    private void validarCamposInvalidezDependente() {
        ValidacaoException ve = new ValidacaoException();
        if (selecionadoPossuiDeficienciaFisica()) {
            if (selecionado.getPessoaFisicaCid().getInicioVigencia() == null) {
                ve.adicionarMensagemDeCampoObrigatorio("Informe o Início da vigência na aba Invalidez do Dependente.");
            }
            if (selecionado.getPessoaFisicaCid().getCid() == null) {
                ve.adicionarMensagemDeCampoObrigatorio("Informe o CID na aba Invalidez do Dependente.");
            }
            if (selecionado.getPessoaFisicaCid().getInicioVigencia() != null && selecionado.getPessoaFisicaCid().getFinalVigencia() != null) {
                if (selecionado.getPessoaFisicaCid().getInicioVigencia().after(selecionado.getPessoaFisicaCid().getFinalVigencia())) {
                    ve.adicionarMensagemDeOperacaoNaoPermitida("A data de Início da vigência não pode ser maior que a data de Final da vigência na aba Invalidez do Dependente.");
                }
            }
    ...
    public void salvar() {
        try {
            if (selecionado instanceof PessoaFisica) {
                ...
                validarCamposInvalidezDependente();
                validarCampos();
            }

            if (isOperacaoNovo()) {
                selecionado.setUnidadeOrganizacional(unidadeOrganizacionalSelecionada.getSubordinada());
                pessoaFacade.salvarNovo(selecionado, arquivo, selecionado.getFileUploadPrimeFaces().getFileUploadEvent());
                carregaFoto();
            }
            ...
            if (isOperacaoEditar()) {
                ...
                pessoaFacade.salvar(selecionado, arquivo, selecionado.getFileUploadPrimeFaces().getFileUploadEvent(), listaExcluidos);
                carregaFoto();
            }
    ...
    private void validarCamposObrigatoriosInformacoesBancarias() {
        ...
        if (Strings.isNullOrEmpty(contaCorrenteBancPessoa.getContaCorrenteBancaria().getNumeroConta())) {
            ve.adicionarMensagemDeCampoObrigatorio("O campo Número da Conta Corrente deve ser informado.");
        }

        if (Strings.isNullOrEmpty(contaCorrenteBancPessoa.getContaCorrenteBancaria().getDigitoVerificador())) {
            ve.adicionarMensagemDeCampoObrigatorio("O campo Dígito Verificador deve ser informado.");
        }

        if (contaCorrenteBancPessoa.getContaCorrenteBancaria().getSituacao() == null) {
            ve.adicionarMensagemDeCampoObrigatorio("O campo Situação deve ser informado.");
        }

        if (contaCorrenteBancPessoa.getContaCorrenteBancaria().getModalidadeConta() == null) {
            ve.adicionarMensagemDeCampoObrigatorio("O campo Modalidade da Conta deve ser informado.");
        }

        try {
            contaCorrenteBancPessoa.realizarValidacoes();
        } catch (ValidacaoException veContaCorrente) {
            if (veContaCorrente.temMensagens()) {
                ve.getMensagens().addAll(veContaCorrente.getMensagens());
            }
        }
    ...
    private void validarContaJaAdicionada() {
        ValidacaoException ve = new ValidacaoException();
        for (ContaCorrenteBancPessoa c : selecionado.getContaCorrenteBancPessoas()) {
            if (StringUtil.cortaOuCompletaEsquerda(c.getContaCorrenteBancaria().getNumeroConta(), 12, "0").equals(StringUtil.cortaOuCompletaEsquerda(contaCorrenteBancPessoa.getContaCorrenteBancaria().getNumeroConta(), 12, "0"))
                && !c.getContaCorrenteBancaria().getId().equals(contaCorrenteBancPessoa.getContaCorrenteBancaria().getId()) && c.getContaCorrenteBancaria().getAgencia().getId().equals(contaCorrenteBancPessoa.getContaCorrenteBancaria().getAgencia().getId())) {
    ...
    public void validarCampos() {
        ValidacaoException ve = new ValidacaoException();
        ValidaPessoa.validar(selecionado, PerfilEnum.PERFIL_DEPENDENTE, ve);

        if (!selecionado.getCpf_Cnpj().equals("")) {
            if (Util.valida_CpfCnpj(selecionado.getCpf_Cnpj())) {
        ...
        } else if (ValidaPessoa.isMaiorDe8Anos(selecionado)) {
            ve.adicionarMensagemDeOperacaoNaoPermitida("O CPF é obrigatório para Dependente com 8(oito) anos ou mais.");
        }
    ...
    private boolean temEnderecoInformado(Pessoa selecionado) {
        if (selecionado.getEnderecos().isEmpty()) {
            FacesUtil.addCampoObrigatorio("É obrigatório informar pelo menos um endereço.");
            return false;
        }
        return true;
    }
    ...
    public void validaCpfRh() {
        ...
        if (!Util.validarCpf((selecionado).getCpf())) {
            ...
            FacesUtil.addMessageWarn("Formulario:iCPF", "Atenção!", "O CPF digitado já pertence a " + getDescricaoConcatenada(pessoaFacade.getPessoasComMesmoCPF(selecionado)));
        }
    }
    ...
    public void validarCpfCompanheiro() {
        ...
        } else if (!getDocumentoPessoalFacade().verificarCpf(certidaoNascimento)) {
    ...
    public void pisValido() {
        ...
        if (!pessoaFacade.validaPisPasep(selecionado, carteiraTrabalho.getPisPasep())) {
    ...
    private void validarEndereco() {
        ...
        if (endereco.getCep() == null || endereco.getCep().trim().isEmpty()) {
            ve.adicionarMensagemDeCampoObrigatorio("O campo CEP deve ser informado! ");
        } else if (endereco.getCep().length() != 8) {
            ve.adicionarMensagemDeOperacaoNaoPermitida("Informe o CEP corretamente.");
        }
        if (endereco.getUf() == null || endereco.getUf().trim().isEmpty()) {
            ve.adicionarMensagemDeCampoObrigatorio("O campo Estado deve ser informado! ");
        } else if (endereco.getUf().length() != 2) {
            ve.adicionarMensagemDeOperacaoNaoPermitida("O campo Estado é uma Sigla é deve ter 2 caractéres");
        }
        if (endereco.getLocalidade() == null || endereco.getLocalidade().trim().isEmpty()) {
            ve.adicionarMensagemDeCampoObrigatorio("O campo Cidade deve ser informado! ");
        }
    ...
    public void removerConselho(ActionEvent evento) {
        ...
        selecionado.getConselhoClasseContratos().remove(c);
    }
    ...
    public PessoaFisicaDependenteControlador getControlador() {
        return pessoaFisicaDependenteControlador;
    }

    public void setControlador(PessoaFisicaDependenteControlador pessoaFisicaDependenteControlador) {
        this.pessoaFisicaDependenteControlador = pessoaFisicaDependenteControlador;
    }
    ...
    public List<Medico> buscarMedico(String parte) {
