...
@URLMappings(mappings = {
    @URLMapping(id = "relatorio-levantamento-bem-por-grupo-bem-comparativo", pattern = "/relatorios/levantamento-bem-por-grupo-bem-comparativo/", viewId = "/faces/administrativo/patrimonio/relatorios/levantamentobemmovel/porgrupobemcomparativo.xhtml")
})
@ManagedBean
public class RelatorioLevantamentoBemPorGrupoBemComparativoControlador extends AbstractReport implements Serializable {
    ...
    @URLAction(mappingId = "relatorio-levantamento-bem-por-grupo-bem-comparativo", phaseId = URLAction.PhaseId.RENDER_RESPONSE, onPostback = false)
    public void relatorioComparativo() {
        limparCampos();
    }
    ...
    public void gerarRelatorio(String tipoRelatorioExtensao) {
        try {
            ...
            dto.adicionarParametro("SECRETARIA", buscarNomeSecretariaComparativo());
            dto.adicionarParametro("MUNICIPIO", montarNomeDoMunicipio());
            dto.adicionarParametro("NOMERELATORIO", nomeRelatorio);
            dto.adicionarParametro("parametrosRelatorio", ParametrosRelatorios.parametrosToDto(montarParametrosEFiltrosRelatorioAcompanhamento(dto)));
            dto.setNomeRelatorio(nomeRelatorio);
            dto.setApi("administrativo/levantamento-bem-por-grupo-bem-comparativo/");
    ...
    private String buscarNomeSecretariaComparativo() {
        if (hoORC != null && dtReferencia != null) {
            HierarquiaOrganizacional hieararquiaAdministrativaDaOrcamentaria = hierarquiaOrganizacionalFacade.recuperarAdministrativaDaOrcamentariaVigente(this.hoORC, getSistemaFacade().getDataOperacao());
            if (hieararquiaAdministrativaDaOrcamentaria != null) {
                HierarquiaOrganizacional secretaria = hierarquiaOrganizacionalFacade.recuperarSecretariaAdministrativaVigente(hieararquiaAdministrativaDaOrcamentaria, dtReferencia);
                if (secretaria != null) {
    ...
    private List<ParametrosRelatorios> montarParametrosEFiltrosRelatorioAcompanhamento(RelatorioDTO dto) {
        ...
        if (hoORC != null && hoORC.getCodigo() != null) {
            ...
            parametros.add(new ParametrosRelatorios(" lev.unidadeorcamentaria_id", ":orcamentaria", null, OperacaoRelatorio.IGUAL, hoORC.getSubordinada().getId(), null, 3, false));
    ...
    private void validarCamposComparativo() {
        ValidacaoException ve = new ValidacaoException();
        verificarCampoNull(ve, dtReferencia, "O campo Data de Referencia deve ser informado.");
        verificarCampoNull(ve, hoORC, "O campo Unidade Orçamentária deve ser informado.");
