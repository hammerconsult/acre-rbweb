 ...
 * @author edi
 */
@ManagedBean(name = "oCCContaExtraControlador")
@ViewScoped
@URLMappings(mappings = {
        @URLMapping(id = "novo-occ-conta-extra", pattern = "/occ/conta-extra/novo/", viewId = "/faces/financeiro/origemcontacontabil/contaextra/edita.xhtml"),
        @URLMapping(id = "editar-occ-conta-extra", pattern = "/occ/conta-extra/editar/#{oCCContaExtraControlador.id}/", viewId = "/faces/financeiro/origemcontacontabil/contaextra/edita.xhtml"),
        @URLMapping(id = "ver-occ-conta-extra", pattern = "/occ/conta-extra/ver/#{oCCContaExtraControlador.id}/", viewId = "/faces/financeiro/origemcontacontabil/contaextra/visualizar.xhtml"),
        @URLMapping(id = "listar-occ-conta-extra", pattern = "/occ/conta-extra/listar/", viewId = "/faces/financeiro/origemcontacontabil/contaextra/listar.xhtml"),})
public class OCCContaExtraControlador extends PrettyControlador<OCCConta> implements Serializable, CRUD {
    ...
    public OCCContaExtraControlador() {
    ...
    @URLAction(mappingId = "novo-occ-conta-extra", phaseId = URLAction.PhaseId.RENDER_RESPONSE, onPostback = false)
    @Override
    public void novo() {
        super.novo();
        setEntidadeOCC(EntidadeOCC.CONTAEXTRAORCAMENTARIA);
        selecionado.setInicioVigencia(sistemaControlador.getDataOperacao());
        origenscc = new ArrayList<>();
    }

    @URLAction(mappingId = "ver-occ-conta-extra", phaseId = URLAction.PhaseId.RENDER_RESPONSE, onPostback = false)
    @Override
    public void ver() {
        super.ver();
        recuperaEditarVer();
    ...
    @URLAction(mappingId = "editar-occ-conta-extra", phaseId = URLAction.PhaseId.RENDER_RESPONSE, onPostback = false)
    @Override
    public void editar() {
        super.editar();
        recuperaEditarVer();
    ...
    private Boolean validaSalvar() {
        ...
        if (origenscc.isEmpty()) {
            FacesUtil.addError(SummaryMessages.OPERACAO_NAO_REALIZADA.getDescricao(),  " Para salvar o registro, é necessário informar uma configuração para a Origem Conta Contábil - Conta de Extraorçamentária. ");
    ...
    public List<Conta> completaContaExtra(String parte) {
        if (sistemaControlador != null) {
            return origemOCCFacade.getContaFacade().listaFiltrandoContaExtraPorTipoDeContaExtra(parte.trim(), sistemaControlador.getExercicioCorrente(), tipoContaExtra);
        }
        return null;
    }
