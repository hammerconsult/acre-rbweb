...
@ManagedBean
@ViewScoped
public class PessoaCorrecaoDadosPesquisaGenerico extends ComponentePesquisaGenerico implements Serializable {
    ...
    public void getCampos() {
        ...
        itens.add(new ItemPesquisaGenerica("nomeFantasia", "Nome Fantasia", String.class));
        itens.add(new ItemPesquisaGenerica("nomeReduzido", "Nome Reduzido", String.class));
        ...
        for (DataTablePesquisaGenerico dataTablePesquisaGenerico : camposPesquisa) {
            if (validaItem(dataTablePesquisaGenerico)) {
                try {
                    ...
                    if (dataTablePesquisaGenerico.getItemPesquisaGenerica().getCondicao().equals("nomeReduzido")) {
                        condicao += " (" + aliasPessoa + " in (select " + aliasPJ + " from PessoaJuridica " + aliasPJ + " where lower(" + aliasPJ + ".nomeReduzido) like '%" + dataTablePesquisaGenerico.getValuePesquisa().toLowerCase() + "%')) ";
                    }
                    if (dataTablePesquisaGenerico.getItemPesquisaGenerica().getCondicao().equals("nomeFantasia")) {
                        condicao += " (" + aliasPessoa + " in (select " + aliasPJ + " from PessoaJuridica " + aliasPJ + " where lower(" + aliasPJ + ".nomeFantasia) like '%" + dataTablePesquisaGenerico.getValuePesquisa().toLowerCase() + "%')) ";
                    }
                    ...
                    if (!somenteAtivas && dataTablePesquisaGenerico.getItemPesquisaGenerica().getCondicao().equals("situacaoCadastralPessoa")) {
                        condicao += " lower(" + aliasPessoa + ".situacaoCadastralPessoa) like '%" + dataTablePesquisaGenerico.getValuePesquisa().toLowerCase() + "%'))";
                    }
                    condicao += " and ";
