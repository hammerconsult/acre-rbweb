...
public class SingletonProtocolo implements Serializable {
    ...
    private Integer recuperaUltimoCodigo(Integer ano) {
        return ((BigDecimal) em.createNativeQuery("select coalesce(max(numero),0) from Processo where ano = :ano and coalesce(protocolo,0) = 1")
