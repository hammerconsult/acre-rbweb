...
@Audited
@Entity
@Etiqueta("Suplementação da Emenda")
public class SuplementacaoEmenda extends SuperEntidade implements Serializable {
    ...
    @Etiqueta("Descrição Sub-Projeto/Atividade")
    @Pesquisavel
    @Tabelavel
    private String descricaoSubProjetoAtividade;
    @ManyToOne
    @Obrigatorio
    @Pesquisavel
    @Tabelavel
    ...
    @Etiqueta("Conta de Despesa Previsa na LOA?")
    private Boolean contaPrevistaLoa;

    public SuplementacaoEmenda() {
        super();
        quantidade = 0;
        valor = BigDecimal.ZERO;
        contaPrevistaLoa = Boolean.TRUE;
    }
    ...
    public String getDescricaoOrSubacao() {
        if (subAcaoPPA != null) {
            return subAcaoPPA.toString();
        } else {
            return descricaoSubProjetoAtividade;
        }
    }
    ...
    public String getDescricaoSubProjetoAtividade() {
        return descricaoSubProjetoAtividade;
    }

    public void setDescricaoSubProjetoAtividade(String descricaoSubProjetoAtividade) {
        this.descricaoSubProjetoAtividade = descricaoSubProjetoAtividade;
    }

    public Boolean getContaPrevistaLoa() {
        return contaPrevistaLoa;
    }

    public void setContaPrevistaLoa(Boolean contaPrevistoLoa) {
        this.contaPrevistaLoa = contaPrevistoLoa;
    }

    public BigDecimal getTotalSuplementacoes() {
        BigDecimal total = new BigDecimal(BigInteger.ZERO);
        if (this.emenda.getSuplementacaoEmendas() != null) {
            for (SuplementacaoEmenda sup : this.emenda.getSuplementacaoEmendas()) {
