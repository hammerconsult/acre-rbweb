...
@URLMappings(mappings = {
        @URLMapping(id = "relatorio-nota-pagamento-estorno-intervalo", pattern = "/relatorio/nota-pagamento-estorno-intervalo/", viewId = "/faces/financeiro/relatorio/relatorionotapagamentoestorno.xhtml")
})
@ManagedBean
public class RelatorioNotaPagamentoEstornoControlador implements Serializable {
    ...
    @URLAction(mappingId = "relatorio-nota-pagamento-estorno-intervalo", phaseId = URLAction.PhaseId.RENDER_RESPONSE, onPostback = false)
    ...
    private String montarCondicao() {
        ...
        stb.append(" and trunc(NOTA.dataestorno) between to_date('").append(DataUtil.getDataFormatada(dataInicial)).append("' ,'dd/MM/yyyy') and to_date('").append(DataUtil.getDataFormatada(this.dataFinal)).append("','dd/MM/yyyy')");
        if (!"".equals(numeroInicial)) {
            stb.append(" and NOTA.numero between '").append(numeroInicial).append("' and '").append(numeroFinal).append("'");
        }
    ...
    public void gerarRelatorio() {
        try {
            validarCampos();
            List<NotaExecucaoOrcamentaria> notas = pagamentoEstornoFacade.buscarNotaDeEstornoPagamento(montarCondicao(), CategoriaOrcamentaria.NORMAL);
