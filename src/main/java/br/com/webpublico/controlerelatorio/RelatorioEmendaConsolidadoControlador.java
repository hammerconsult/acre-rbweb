...
@URLMappings(mappings = {
    @URLMapping(id = "relatorio-emenda-consolidado", pattern = "/relatorio-emenda-consolidado/", viewId = "/faces/financeiro/relatorio/relatorio-emenda-consolidado.xhtml")
})
@ManagedBean
public class RelatorioEmendaConsolidadoControlador implements Serializable {
    ...
    @URLAction(mappingId = "relatorio-emenda-consolidado", phaseId = URLAction.PhaseId.RENDER_RESPONSE, onPostback = false)
    ...
    public void gerarRelatorio() {
        try {
            ...
            dto.setNomeRelatorio("RELATORIO-EMENDA-CONSOLIDADO");
            dto.setApi("contabil/emenda-consolidado/");
    ...
    private String montarParametros() {
        String retorno = " where trunc(em.dataEmenda) between to_date('" + DataUtil.getDataFormatada(dataInicial) + "', 'dd/MM/yyyy') and to_date('" + DataUtil.getDataFormatada(dataFinal) + "', 'dd/MM/yyyy') ";
        ...
        if (statusCamara != null) {
            retorno += " and em.statuscamara = '" + statusCamara.name() + "' ";
            filtros += " Status da Câmara: " + statusCamara.getDescricao() + " -";
        }
        if (statusPrefeitura != null) {
            retorno += " and em.statusPrefeitura = '" + statusPrefeitura.name() + "' ";
