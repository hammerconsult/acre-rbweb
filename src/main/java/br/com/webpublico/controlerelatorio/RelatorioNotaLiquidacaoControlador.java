...
@URLMappings(mappings = {
    @URLMapping(id = "relatorio-nota-liquidacao-intervalo", pattern = "/relatorio/nota-liquidacao-intervalo/", viewId = "/faces/financeiro/relatorio/relatorionotaliquidacao.xhtml")
})
@ManagedBean
public class RelatorioNotaLiquidacaoControlador implements Serializable {
    ...
    @URLAction(mappingId = "relatorio-nota-liquidacao-intervalo", phaseId = URLAction.PhaseId.RENDER_RESPONSE, onPostback = false)
    ...
    private String montarCondicao() {
        ...
        stb.append(" and trunc(coalesce(nota.dataliquidacao, nota.dataregistro)) between to_date('").append(DataUtil.getDataFormatada(dataInicial)).append("','dd/MM/yyyy') and to_date('").append(DataUtil.getDataFormatada(dataFinal)).append("','dd/MM/yyyy') ");
        if (!this.numeroInicial.equals("") && !this.numeroFinal.equals("")) {
            stb.append(" and ").append(" nota.numero between '").append(this.numeroInicial).append("' and '").append(this.numeroFinal).append("'");
        }
    ...
    public void setLiquidacaoFacade(LiquidacaoFacade liquidacaoFacade) {
        this.liquidacaoFacade = liquidacaoFacade;
    }
